...
import synth.models.Particles.{HeatedAllScaleParticleProc, DES, HeatedParticleSystemProc, NoteEvent}
...
class ScalePicker(fundamental: Float) extends BoxPanel(Orientation.Vertical) {
  protected lazy val scales = Map[String, () => TypedScale](
    "Full (Pythagorean)" -> {
...
class FullScalePicker(fundamental: Float) extends ScalePicker(fundamental) {

  override protected lazy val scales = Map[String, () => TypedScale](
    "Full (Pythagorean)" -> {
      () => ScaleBuilderPythagFull(fundamental).build
    },
    "Full (Even Temperment)" -> {
...
object DemoModulatingParticles extends SimpleSwingApplication {
    ...
    case ButtonClicked(button) => {
      if (button == playButton) {
        ...
        try {
          ...
          case _: Throwable => System.err.println("could not play the song in this scale")
        }
        button.enabled = true
      }
    }
  ...
  val scalePicker = new FullScalePicker(528f)
